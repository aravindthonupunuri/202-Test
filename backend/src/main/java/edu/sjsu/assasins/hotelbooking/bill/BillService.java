...
import edu.sjsu.assasins.hotelbooking.models.*;
...
public class BillService {
    ...
    public ResponseEntity<Object> makePayment(Bill bill){
        ...
        Customer customer = customerRepository.findCustomerByEmail(associatedBooking.getCustomerEmail()).get(0);
    ...
    public static void setBillAttributes(Bill bill, Booking booking){
        ...
        bill.setTaxAmount(totalBillAmount * 0.12 ); //Using 12 percent tax
        bill.setTotalPayableAmount(bill.getTotalAmount() + bill.getTaxAmount());
        Customer customer = customerRepository.findCustomerByEmail(booking.getCustomerEmail()).get(0);
        int availableRewardPoints = customer.getRewardPoints();
